...
def generate_launch_description():
    ...
    hri_dir = get_package_share_directory('hri')
    gazebo_models_dir = os.path.join(hri_dir, 'models')

    # Launch configuration variables specific to simulation
    turtlebot_count = 3  # Number of TurtleBots to spawn
    turtlebot_models = ['waffle', 'waffle', 'waffle']  # Models for each TurtleBot
    turtlebot_namespace = ['waffle1', 'waffle2', 'waffle3']  # Models for each TurtleBot
    x_poses = ['-2.0', '4.0', '-6.5']  # X positions for each TurtleBot
    y_poses = ['1.0', '1.0', '-1.0']  # Y positions for each TurtleBot
    yaw_angles = ['0.0', '0.0', '0.0']  # Yaw angles in radians for each TurtleBot (0, 90 degrees)
    ...
    for i in range(turtlebot_count):
        ...
        spawn_entity_cmd = Node(
            ...
            arguments=[
                ...
                '-z', '0.01',
                '-Y', yaw_angle,
                '-robot_namespace', turtlebot_namespace[i],
            ],
            output='screen'
        )
